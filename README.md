...
| **[Learn RunPod LLM Serving](https://github.com/Syntran-Labs/learn-runpod-llm-serving)** | LLMOps / GPU Serving | ✅ Published | Deploy and benchmark a 20B-class open-weights LLM (GPT-OSS-20B) on an on-demand RunPod GPU pod ($0.22-0.27/hr): OpenAI-compatible serving, SSH-tunneled access with zero public ports, gated lifecycle scripts, reproducible benchmarking and cost-per-token analysis (~$0.32-0.43/M generated tokens), and honest ops notes (RUNBOOK, SESSION-LOG, KNOWN-ISSUES). A Learning Lab project with Systems Lab-grade operational discipline — educational reproducibility, not a managed production service |
